...
interface MiniChartProps {
  /** Use real data if available, otherwise generates synthetic from baseValue */
...
function generateSyntheticData(base: number, change: number, points = 48): { time: number; value: number }[] {
  ...
  let v = base * (1 - (change / 100) * 0.5);
  for (let i = 0; i < points; i++) {
    const progress = i / points;
    const noise = (Math.random() - 0.5) * base * 0.02;
    const trend = direction * progress * base * Math.abs(change / 100) * 0.8;
    v = Math.max(0.0000001, base * (1 - (change / 100) * 0.5) + trend + noise);
    data.push({ time: now - (points - i) * 30 * 60_000, value: v });
  }
  return data;
}

export function MiniChart({ data, baseValue = 1, change = 0, height = 80, type = "area", label, className }: MiniChartProps) {
  ...
  const fillId = `miniGrad-${Math.random().toString(36).slice(2, 8)}`;

  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      <div>
        ...
        <p className="text-[10px] sm:text-xs font-mono text-muted-foreground">Holdings & performance analytics</p>
      </div>
      ...
      {priceEntries.length > 0 && (
        <PanelShell title="Token Allocation" subtitle="Relative positions">
          <MiniChart
            data={priceEntries.map(([, p], i) => ({ time: i, value: p.price }))}
            baseValue={1}
            change={0}
            height={80}
            type="bar"
        ...
        <PanelShell title="Tracked Positions" subtitle={`${items.length} tokens`} noPad>
                    ...
                    <tr key={item.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                      <td className="p-3">
                        <Link to={`/token/${item.address}`} className="flex items-center gap-2 hover:text-primary">
                          <div className="h-6 w-6 rounded-full bg-primary/10 flex items-center justify-center text-[8px] font-bold text-primary shrink-0">{(item.label ?? "?").slice(0, 2)}</div>
                      ...
                      <td className="p-3">
                        <div className="w-24 mx-auto">
                          <MiniChart baseValue={pd?.price ?? 1} change={pd?.change24h ?? 0} height={32} />
                        </div>
                      </td>
                      <td className="p-3 text-left hidden lg:table-cell text-muted-foreground">{item.address.slice(0, 12)}…</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div className="sm:hidden divide-y divide-border/50">
            {items.map(item => {
              const pd = prices?.[item.address];
              return (
                <Link key={item.id} to={`/token/${item.address}`} className="flex items-center gap-3 p-3 hover:bg-muted/20 transition-colors">
                  <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-[9px] font-bold text-primary shrink-0">{(item.label ?? "?").slice(0, 2)}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-mono font-medium text-foreground truncate">{item.label || "Unknown"}</p>
                    <p className="text-[10px] font-mono text-muted-foreground">{pd ? formatPrice(pd.price) : "—"}</p>
                  </div>
                  <div className="w-16 shrink-0">
                    <MiniChart baseValue={pd?.price ?? 1} change={pd?.change24h ?? 0} height={28} />
                  </div>
                  <span className={cn("text-[10px] font-mono shrink-0", pd ? (pd.change24h >= 0 ? "text-terminal-green" : "text-destructive") : "text-muted-foreground")}>
                    {pd ? `${pd.change24h >= 0 ? "+" : ""}${pd.change24h.toFixed(1)}%` : "—"}
                  </span>
                </Link>
              );
